    ...
    <ModalShell width={480} onBackdropClick={() => close()}>
      ...
      {currentPlanKey && (
        <p className="hint-muted">
          ...
          <strong style={{ color: 'var(--teal-dark)' }}>二重に請求されることはありませんので、安心してご変更いただけます。</strong>
        </p>
      )}
...
const usageGuideSections: { heading: string; body: string }[] = [
  { heading: '① 利用者を追加', body: '左側の利用者一覧の「利用者を追加」から、お名前を登録します。' },
  { heading: '② 留意点を追加(任意)', body: '既往歴や注意事項など、あらかじめ知っておきたいことがあれば「留意点を追加・編集」から記録しておけます。' },
  { heading: '③ モード選択で所見項目を調整', body: '右上の「モード選択」から、所見の項目を施設に合わせて追加・削除できます。' },
  {
    heading: '④ 今月の所見を入力',
    body: 'プルダウンでの選択・自由記入欄への入力のどちらでも構いません。両方空欄のままでも生成できるので、無理にすべて埋める必要はありません。\n「前月の所見をコピー」から前月の入力内容をそのまま引き継ぐこともできますが、今月の実際の様子と照らし合わせて内容を編集してから生成してください。\nパソコン・タブレットでは、項目左端の「☰」をドラッグして並び順を入れ替えられるほか、「過去の記録の追加・編集」から過去3か月分のモニタリングを入力しておくと、AIによる生成の精度が上がります(いずれもスマホでは非対応です)。',
  },
  { heading: '⑤ 文章を生成', body: '「文章を生成」ボタンでAIが下書きを作成します。生成後の文章は自由に編集できます。' },
  { heading: '⑥ コピーして貼り付け', body: '「文章をコピー」でクリップボードにコピーし、お使いの記録システムなどへ貼り付けてください。内容を確認できたら「確認済み」にチェックを入れておくと、利用者一覧にチェックマークが付き、その月の作業が済んだ人を把握しやすくなります。' },
  {
    heading: 'その他',
    body: '・右上の「今月分を出力」から、対象年月の全利用者分の生成結果をまとめてテキストファイルとして書き出すこともできます。\n・パソコン・タブレットのログイン画面の「スマホでログイン」に表示されるQRコードをスマホのカメラで読み取ると、そのままスマホでログインできます。',
  },
  {
    heading: 'ご注意',
    body: '・AIによる文章生成の際、利用者の氏名は送信されません。ただし、留意点欄や自由記入欄に氏名等を記載した場合は、その内容がそのまま送信されますのでご注意ください。\n・個人として業務上のご利用をされる場合は、あらかじめ所属先の管理者等の許可を得たうえでご利用ください。',
  },
];
